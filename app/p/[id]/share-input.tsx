...
export default function ShareInput({ url, title }: ShareInputProps) {
  const copyToClipboard = () => {
    navigator.clipboard.writeText(url);
    const button = document.getElementById('copy-input-btn');
    if (button) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-sm text-gray-600">
        <p className="mb-2 font-semibold">Share this paste:</p>
        <input
          type="text"
          readOnly
          value={url}
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <button
          ...
          className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded text-sm transition-colors flex items-center gap-2 shadow-sm"
          title="Copy URL"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
